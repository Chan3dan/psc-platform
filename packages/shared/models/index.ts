...
// File: packages/shared/models/index.ts
// ============================================================
...
export interface IExam extends Document {
  ...
  negative_marking: number; // e.g. 0.25 means -0.25 per wrong
...
export interface ISubject extends Document {
  ...
  description?: string;
  question_count: number; // denormalized for perf
  is_active: boolean;
}
...
export interface IQuestion extends Document {
  ...
  year?: number; // PSC exam year if from past paper
  tags: string[];
  is_active: boolean;
...
export interface IMockTest extends Document {
  ...
  config: {
    question_ids?: Types.ObjectId[]; // if curated
    auto_generate: boolean; // or auto from subjects
...
// ─────────────────────────────────────────────
// 6. RESULT SCHEMA (exam attempt)
// ─────────────────────────────────────────────
export interface IAnswer {
  question_id: Types.ObjectId;
  selected_option: number | null; // null = skipped
  is_correct: boolean;
        ...
        tasks: [
          {
            ...
            minimum_questions: Number,
            minimum_minutes: Number,
            verification_mode: { type: String, enum: ['questions', 'mock', 'revision', 'notes'] },
            verified_questions: { type: Number, default: 0 },
            verified_minutes: { type: Number, default: 0 },
            verification_source: { type: String, default: '' },
...
// ─────────────────────────────────────────────
// 8. STUDY SESSION + REVISION LOG SCHEMAS
// ─────────────────────────────────────────────
...
const StudySessionSchema = new Schema<IStudySession>(
  {
    ...
    subject_id: { type: Schema.Types.ObjectId, ref: 'Subject' },
    topic_slug: { type: String, required: true, lowercase: true },
...
const NoteSchema = new Schema(
  {
    ...
    content_url: String, // Cloudinary URL for PDF
    content_html: String, // rich text
